...
public abstract class MyTabActivity extends TabActivity {
	...
	private void initTabHost() {
		...
		int index = 0;
		// Create TabSpec for each MyTab. The first tab_activity is the default
		...
		// Listener on tab_activity change
		tabHost.setOnTabChangedListener(new OnTabChangeListener() {
			@Override
			public void onTabChanged(String tabId) {
				Log.d(TAG_NAME, "change tab_activity: id=" + tabId + ", prevId=" + tabViewTagPrev);
				if (tabViewTagPrev != null) {
					// Reset prev tab_activity
				...
				// Set current selected tab_activity
